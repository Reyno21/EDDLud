...
import correos.Cuenta;
import estructuras.MultiListas;
...
public class NuevaCuenta extends javax.swing.JDialog
{
   ...
   public String corr;
     ...
     * @param modal
     */
    public NuevaCuenta(java.awt.Frame parent, boolean modal, String correo)
    {
        super(parent, modal);
        initComponents();
        cjb.ci.CtrlInterfaz.habilita(false, btnAceptar);
        this.padre = (MenuCorreo) parent;
        this.corr = correo;
        if (correo.compareTo("") != 0)
        {
            jTextGmail.setText(correo);
            jTextGmail.setEnabled(false);
        }
    }
    ...
    {//GEN-HEADEREND:event_btnAceptarActionPerformed
        ...
        if (ptr.matcher(correo).matches() && pass1.compareTo(pass2) == 0)
        {
            if(this.corr.compareTo("") !=0)
            {
                Nodo aux = MultiListas.busca(MenuCorreo.datos.getContenido(), corr);
                if (aux != null)
                {
                    Cuenta obj = (Cuenta) aux.getObj();
                    obj.setContrasenia(pass1);
                }
            }else{
                Cuenta obj = new Cuenta(correo, pass1);
                Nodo aux = new Nodo(obj, correo);
                String[] etqs = new String[1];
                etqs[0] = correo;
                MenuCorreo.datos.setContenido(MultiListas.inserta(MenuCorreo.datos.getContenido(), aux, 0, etqs));
                padre.mostrarCuentas();
            }
        }
